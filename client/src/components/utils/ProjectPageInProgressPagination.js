...
export default function ProjectPageInProgressPagination({
  ...
  const handleButton = async (event) => {
    ...
    const { data } = await axios({
      method: "GET",
      url: `${process.env.REACT_APP_API_URL}/project/${userId}/?state=progress&page=${newPageNum}`,
      withCredentials: true,
    });

    setInProgressProjects(data.data);
  };
